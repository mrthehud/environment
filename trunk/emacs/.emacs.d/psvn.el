    ...
    (cond ((string= event "finished\n")
           ...
           (cond ((eq svn-process-cmd 'status)
                  ...
                  (when svn-status-update-previous-process-output
                    (set-buffer (process-buffer process))
                    (delete-region (point-min) (point-max))
                    (insert "Outpu                    (insert svn-                        (when svn-status-update-li                    ;; (message "Using svn-status-upd                    (save-excursion
                      (svn-status-update-wit                        (when svn-status-disp                    (set-wi                          (message "svn status finished")
                      (switch-to-buff                        (svn-status-show-process-output 'log t)
                  (pop-to-buffer svn-status-last-output-buffer-name)
                  (svn-log-view-mode)
                  (forward-line                     (forward-line 1))
                  (font-lock-font                  (message "svn log finished"))
                 ...
                 ((eq svn-process-cmd 'commit)
                  ...
                  (if (null svn-status-commit-rev-number)
                    ...
                    (message "svn: Committed revision %s." svn-status-                        (svn-status-show-process-output 'upd                  (setq svn-status-update-list (svn-status-parse-update-output))
                  (svn-revert-some-buffers)
                  (svn-status-update)
                  (if (car svn-st                      (message "svn: Update                    (mes                          (svn-status-update)
                  ...
                  (when (member 'revert svn-status-unmark-files-after-list)
                    (svn-status-unset-                  (svn-revert-some-buffers                        (message "svn reve                        (svn-status-update)
                  (message "svn resolved finis                 ((eq svn-process-                     (message "svn rm finished"))
                 ...
                 ((eq svn-process-cmd 'proplist)
                  ...
                  (message "svn proplist finished                        (svn-status default-directory))
...
(defun svn-process-filter (process str)
  ...
  (save-window-excursion
    ...
    (goto-char (p     (      (goto-char (svn-point-at-bol))
      (when (looking-at "Password for '\\(.+\\)': ")
        ...
        (let ((user-name (read-string "Username for svn operation: ")))
          (svn-process-send-string-and-newline user-nam      (when (looking-at "(R)eject, accept (t)emporarily or accept (p)erm        (svn-status-show-process-buffer)
        (let ((answe
...
USER-ELIDE i  from the svn-status buffer, typically with `  To r
Call `svn-status-line-info->ui-status' to access the whostructure."
  (list nil nil))
...
of 'svn status'"
  ;; (messa  (let ((candidate)
        (base-dir))
    (dol      (setq base-dir (file-name-directory dir))
      (while base-dir
              (setq candidate (replace-regexp-in-s          ;; (message "svn-status-make                                  dir
                     (gethash dir old-ui-information)))
                       svn-status-info))

(defun svn-status-make-                                        ui
                                  ...
                                  locked-repo-mark
                                  psvn-extra-info)
  "Create a new line-info from the Anythnb: LOCKED-MARK refers to the kind of locks you get after an error,
...
To add support for the names \"feng shui\" and \"mister blank\", place the following in your .emacs:
 ...
 (add-hook 'svn-pre-parse-status-hook 'svn-status-parse-fixup-us")
;;(setq svn-user-names-including-blanks '("feng shui" "mister blank"))
...
(defun svn-status-parse-fixup-user-names-including-blanks ()
  "Helper functionAdd th`svn-user-names-including-blbthe u  (when svn-user-names-including-blanks
    (goto-char (point-min))
    (let ((search-string (concat " \\(" (ma                (while (re-search-forward search-string (point-max) t)
            (replace-match (
(defu  "Parse the `svn-process-buffer-naT  (set  (save-excursion
    (let ((old-ui-information (svn-status-ui-information-hash-table))
          ...
          (svn-marks-length (if svn-status-verbose
                                (if svn-status-remote
                                    8 6)
                             (run-hooks 'svn-pre-p            (while (< (point) (point        (cond
         ((= (svn-point-at-eol) (svn-                 ;; the above messag                 ((looking-at "Performing s                ;; [ie the directory in (match-string 1)]
         ...
         (t
          ...
          (if (and skip-double-external-dir-entry-name (string= skip-double-external-dir-entry-name path))
              ...
              (let ((info (gethash path externals-map)))
                ;; (message "skip-double-external-dir-entry-name: %s - path: %s" skip-do                (if info
                    (progn
          ...
          (setq author-width (max author-width        (forward-line 1))
      (unless svn-status-verbose
        (setq svn-status-info (svn-status-                                                  (setq svn-status-defa                     (if svn-status-short-mod-flag-p 3 0)))
      (s                                                                     (setq svn-status-inf        
;;(string-lessp "." "%") => nil
;(svn-status-sort-predicate '(t t t ".") (defun svn-status-sort-predicate (a b)
  "ReturnA and B must be line-info'  (stri                (concat (svn-status-line-i
(defun svn-status-remove-temp-file-maybe ()
  "Remove any (no longer required) temporary files created by psvn.el."
          (when (file-exists-p svn-status-temp-arg-file)          (s
(defun svn-sta  "Rem  (inte  (let    (sa             (while (re-search-forward "\r$" (point-max) t)
          
(condition-case nil
    ;;(easy-menu-add-item nil '("tools") ["SVN Status" svn-status t] "PCL-CVS")
    (easy-menu-add-item nil '("tools") ["SVN Sta (defvar(put 'svn-status-mode-map 'risky-l(def  "Sub(put 's(de  "Su(put 'svn-status-mode-property-map  'risky-local-variable t)(defva  "Subkeymap used in `svn-status-mode' for option commands.")
(put 'svn-status-mode-options-map   'ri(  "Subk(put 's(defvar svn-status-mod  "Su(put 'svn-status-mode-extension-map 'risky(defvar svn-status-mode-branch-map ()
  "Subkeymap used in `svn-status-mode' for branching commands.")
(put 'svn-status-mo(defvar  "Subkeymap used in `svn-status-mode' for search commands.")
(put 'svn-status-mode-search-ma
(when   (setq svn-status-mode-map (make-  (sup  ;; Don't use (kbd "<return>"); it's unre  (def  (define-key svn-status-mode-map (kbd "<mo  (def  (define-key svn-status-mode-map (kbd "h") 'svn-s   (de  (define-key svn-status-mode-map (kbd "C-o")   (def  (define-key svn-status-mode-map (kbd "e") 'svn-sta     (def  (define-key svn-status-mode-map (kbd "m") 'svn-status-set-user-mark)
  (define-key svn-status-mode-map (k  ;; This matches a binding of `dired-unmark-all-files' in `dired-mode-map'
  ;; of both GNU Emacs and XEmacs  ;;   ;; Or you could just use "*!".
  ...
  ;; `delete-key-deletes-forward' option.  We don't use those,   ;; reasons: psvn.el may be loaded before user customizations, and
  ...
  (define-key svn-status-mode-map (kbd "M-c") 'svn-status-cleanup  (define-key svn-status-mode-map (kbd "k") 'svn-status-lock)
  (define-key svn-status-mode-map (kbd "K") 'svn-status-unlock)
  (define-key svn-status-mode-map (kbd "U") 'svn-status-update-cmd)
  (define-k  (define-key svn-status-mode-map (kbd "r") 'svn-status-revert)
  ...
  (define-key svn-status-mode-map (k
  (define-key svn-status-mode-map (kbd "n") 'svn-status-next-line)
  (define-key svn-status-mode-map (kbd   (define-  (define-key svn-status-mode-map (kbd "  (define-key svn-status-mode-map (kbd "C-x C-j") 'svn-status-dired-jump)
...
(when (not svn-status-mode-mark-map)
  ...
  (define-ke  (d  (  
(whe    (    (
(wh  (setq svn-status-mode-property-map (make-sparse-keymap))
  ...
  '("SVN"
    ...
    ("Property"
     ...
     ["Edit svn:ignore Property" svn-status-property-edit-svn-ignore t]
        ["Edit svn:keywords List" svn-status-pr     ["Add/Remove Id to/from s     ["Add/Remove Date to/from svn:keywords" svn-status     "---"
     ["Select svn:eol-style" svn-status-property-set-     ["Set svn:executable         ("Options"
     ["Save Options" svn-status-save-state t]
     ["Load Options" svn-status-load-s     ["Set Trac project root" svn-status-set-trac-pro     ["Set Short module     ["Set Changelog style" svn-status-set-changelog-style t]
      ["Sort      :style toggle :selected svn-status-sort-status-bu     ["Use -v for svn status calls"       ["Display f      :style toggle :selected svn-stat     )
    ("Trac"
     ...
     ["Browse ticket" svn-trac-br     ["Browse changeset" svn-trac-browse-changeset t]     ["Set Trac project root" svn     )
    "---"
    ["Edit Next SVN Cmd Line" svn-status-toggle-edit-cmd-flag t]    ["Work Directory History..." svn-status-    ("Mark / Unmark"
     ["M     ["Unmark" svn-status-unset-user     ["Unmark all" svn-status-unset-all-usermarks       [     ["Mark/Unmark modified" svn-status-mark-modif     ["Mark/Unmark mo     ["Mark/Unmark added" svn-status-mark-added t]     ["Mark/Unmark deleted     ["Mark/Unmark modified/added/deleted" svn-statu     ["Mark/Unmark file     ["Mark/Unmark filename by     )
    ["Hide Unknown" svn-status-toggle     :style toggle :selected svn-status-hide-unkno    ["Hide Unmodified" s     :style toggle :selected svn-status-hide-unmod    ["Show Client versio    ["Prepare bug report" svn-prepare-bug-r    ))

(defvar svn-status-file-popup-menu-list
  '(["open" svn-status-find-file-other-window t]
    ["sv    ["svn commit" svn-status-commit t]    ["svn log" svn-status-show-svn-log t]
    ["svn     [    ["unmark" svn-status-unset-user-mark t]
    ["svn add" svn-status-    ["svn add recursively" svn-status-add-fi    ["svn mv..." svn-st    [    ["svn lock" svn-status-lock t]
    ["svn unlock" svn-status-unlock t]
    ["svn info" svn-status-info t]
  ...
  (let* ((line-info (svn-status-get-line-information))
    ...
    (when line-info
      ...
      (svn-status-face-set-temporary-during-popup
       'svn-status-marked-popup-face (svn-point-at-bol) (svn-poin       svn-status-actual-popup-menu))
(defun svn-status-face-set-temporary-during-popup (face begin end menu &optional prefix)
  "Put FACE on BEGIN and END in the buffer during Popup PREFIX is passed to `popup-m  (let    (unwind-protect
        (progn
...
(defun svn-status-line-info->is-visiblep (line-info)
  ...
  (or (not (or (svn-status-line-info->hide-because-unknown line-info)
               ...
               (svn-status-line-info->hide-because-custom-hide-function line-               (svn-status-line-info->hide-beca      (svn-status-line-info->update-available line-info) ;; show the line, if an update i      ))

(defun svn-status-line-info->hide-because-unknown (line-info)
  (and svn-status-hide-unknown
       (eq (svn-status-line-info-
...
(defun svn-status-line-info->hide-because-unmodified (line-info)
  ;;(mess  ;;         (svn-status-line-info->filemark line-info) (svn-status-line       (and (or (eq (svn-status-line-info->f                (or (eq (svn-status-line-info->propmark line-  
(de  (eq (svn-status-line-info->user-
(defun svn-status-line-info->show-user-elide-continuation (line-info)
  (eq (svn-status-line-info->user-elide line-i
...
(defun svn-status-line-info->set-ps  (
(defun svn-status-copy-current-line-info (arg)
  "Copy the current fiIf no file is at point, copy everything starting fr  (interactive      (save-excursion
      (goto-char (svn-point-at-bol))
      (when (l        (kill-new (svn-match-string-no-properties 1))
        (message "Copied: %s" (svn-match-string-no-properties 1))))))

(defun  "CWhen called with the prefix argument 0, use the fu  (interactive "P")
  (let ((s                 (svn-st              (message "Copied %s" str)))

(defun svn-status-get-child-directories (&optional dir)
  "Return a list of subdirectories for DIR"
  (interactive  (let ((this-dir (conc          (sub-dir-lis            (setq test-dir (svn-status-line-info->fu        (when (string= (file-name-directory test-dir) this-dir)
          (add-to-list 'sub-dir-list (fi    sub-dir-list))
  ...
  (let ((tag-string))
    ...
    (cond ((equal action 'committed)
          ...
          ((equal action 'deleted)
           ...
           (setq tag-string " <rep                             ((equal action 'confli                  (setq tag-string " <merged>"))
          ((equal action 'propset)
           ;;(setq                 (svn-status-line-info->set-f           (svn-status-line-info->set-localrev line-info 0))
          ((equal action 'deleted-wc)
           (svn-stat               (    (when (and tag      (svn-statu      (svn-status-    (let ((buffer-read      (delete-reg            (when tag-string
        (insert tag-string))
      (delete-char 1))))
...
(defun svn-s  "Parse the output of svn commitReturn a list that is  (sa    (set-buffer svn-process-buffe    (l          (file-name)
          (skip)
          (result))
      (goto-char (point-min))
      (setq svn-st      (setq skip nil) ; set to       (while (< (point) (point-max))
        (cond ((= (svn-point-at-eol) (svn-point-at-               (setq skip t))
                                   (setq action 'added))                       (setq action 'replaced))                      (setq svn-status-commit-rev-number
                     (string-to-number (svn-match-string-no-properties 1)))
                               (forward-char 15)
                    (search-forwa               (unless svn-status-last-commit-author
            (setq svn-status-last-commit-author (car (svn-status-info-for-path (               (setq sk        (forward-li      result)))
;;(svn-status-parse-commit-output)
;;(svn-status-annotate-status-buff
(defun svn-status-parse-ar-out  "Parse the output of svn add     (s    (let ((action)
          (name)
          (skip)
          (result))
      (goto-char (po                      ((looking-at "A")
               (setq action 'added-wc))
              ((looking-at "D")
               (setq action 'delet             (unless skip ;found an int                 (setq name (buffer-substring-no-proper          (setq result (cons (list name a              result)))
;; (svn-status-parse-ar-output)
;; (svn-status-update-with-command-list
(  "PaReturn a list that is suitable for `svn-status-update-with-command-list'"
  (save-excursion
    (set-buffer svn-process-buffer-name)
    ...
    (let ((action)
      ...
      (goto-char (point-min))
      (while (<                     (setq skip t))
              ((look                     ((looking-a               (setq svn-status-update-rev-number
                     (list nil (string-to-n               (setq skip t))
              ((looking-at "U")
               (setq action 'updated))
              ((looking-at "A")
               (              ((looking-at "D")
              ...
              (t ;; this should never be needed(?)
               (setq action (con              (unless skip ;found an interesting line
                  (setq name (          result)))
;; (sv;; (svn-status-update-with-command-list (svn-statu
(defun svn-statu  "PaReturn a list that is suitable for `svn-status-update-with-command-lis  (sav    (set-buffer svn-process-buffer-name)
    (let      (dolist (line (split-string (buffer-sub        (message "%s" line)
        (when (string-match "property '\\(.+\\)' set on '\\(.+\\)'" line)
          ;;(message "property %s - file %s" (match-string 1 li            result)))
...
by subversion on such systems."
  ;; on win32 would need to see how svn does symli  (file-symlink-p (svn-status-line-info->filename line-info)))

(defun svn-status-line-info->directory-p (line-info)
  "Return t if LINE-INFO refers to a directory, nil Symbolic links to directories count as directories (see `file-directory-p')."
  (file-directory-p (svn-status-line-info->fil
...
(defun sv  "Return number of \"/\"'s in STRING."
  (let ((n 0)
        (last 0))
    (while (setq last     n))

(defun svn-insert-line-in-status-buffer (line-info)
  "Format LINE-  (let ((usermark (if (svn-status-line-info->has-usermark line-info) "*" " "))
        (update-available (if (svn-status-line-info->update-available line-                                                                              (if svn-                         (concat
                                         (svn-add-face                           (let ((dir-                                                  line-in                                             (                                       dir-name))
                           'svn-status-directory-face)
                        ;; showing                         (make-string (* 2 (svn-s                                           (svn-status-line-info->filename line-info)))
                                     32))
                      ;;symlinks get a different face
                      (let ((target (s                        (if target
                            ;; name -> trget
                          ...
                          ;; else targe                          (svn-sta                           (svn-status-line-info->directory-p line-info)
                           (svn-status-line-info->filename-nondirectory line-info)
                           'svn-status-directory-face
                           'svn-                            (elide-    (s                 (point)                 svn-statu             (svn-status-                          (format svn-status-line-format
                             (svn-status-line-info->filemark line-info)
                             (or (svn-status-line-info->propmark line-info) ? )
                             (or (svn                                                           (when svn-status-short-mod-                     fil                           (svn-status-maybe-add-string (svn-status-line-info->locked line-                                                                                               (let ((fla                                                          ((eq flag ?O) " [ REPO-LOCK-OTHER ]")
                                                                                           (t " [ REPO-LOCK-UNKNOWN ]")))
...
Additionally clear the psvn-extra-info field in all line-info lists."
  (interactive)  (d    (svn-status-line-info->set-psvn-extra-info line-inf  (svn-status-update-buffe
...
(defun svn-status-checkout-prefix-path ()
  ...
  (let ((root (svn-status-base-info->repository-root))
    ...
    (setq    (      (message "svn-status-checkout-prefix-path: '%s'" p))
    p))

(defun svn-status-ls (path &optional synchron)
  "Run svn ls PATH."
  (interactive "sPath for svn l  (svn-run (not synchron) t 'ls "ls  (when synchron
    (split-string (with-current-buffer svn-process-buffer-name
                    (buffer-substring-no-pro
(defu  "Show, which branches exist for Note: this command assumes the prop  (interactive)
  (svn-  (svn-
(defun svn-status-ls-tags ()
  "Show, which tags Note: th  (interactive)
  (svn-status-parse-info t)
...
The string in parentheses is shown in the status line to show the state."
   (co         (setq svn-status-edit-svn-command nil))
        ((eq svn-status-ed        (con         (setq svn-status-mode-line-process-edit-flag " Edit        ((eq svn-status-edit-svn-command 'st         (setq svn-status-mode-line-process-ed   (sv
(defun svn-status-goto-root-or-return ()
  "Bounce point between the roo  (interactive)
  (if (string= (svn-status-line-info->f      (when svn-status   
...
When called with a prefix argument go back the given number of lines."
  ...
  (while (progn
           (forward-line (- nr-of-lines))
           (and (not (bobp))
                (not (svn-  (when (svn-status-get-line-information    (goto-char (+ (svn-point-at-bol) svn-status-default-column))))

(defun svn-status-dired-jump ()
  "Jump to a dired buffer, containing the file at point."
  (interactive)
  (let* ((line-info (svn-status-get-line-information))
         (file-full-path (                                de    (l            (file-name-as-direct             (expand-file-name (if line-info
                                 (
(defun svn-status-possibly-negate-meaning-of-arg (arg &optional command)
  "Negate arg, if this-command is a member of svn-status-possibly-nega    (if       (not arg)
    arg))

(defun svn-status-update (&optional arg)
  "Run 'svn status -v'.
When called with a prefix argument run 'svn status -vu'."
  (i     (      (set-buffer svn-process-buffer-na      (            (buffer-substring (poi  (svn-status default-directory arg))

( The r  (if (eq major-mode 'svn-status-mode      (let ((svn-        (dolist (overlay (overlays-at (point)))
          (setq svn-info (or svn-info
                                      ;; different mode, means called not from the     (if svn-status-get-line-information-for-file
...
See `svn-status-marked-files' for what counts as selected."
  (if use-marked-files
    ...
    (list (svn-statu
(defun svn-status-get-file-list-names (use-mar  (mapca
(defun svn-status-get  "FiThe result may be parsed with the varWWhen c  (cond ((eq major-mode 'svn-status-mode)
         (svn-status-get-line-inform        (t
         ;; a fake strukture that contains the buffername for t
...
directory itself, in all other cases find the parent directory"
  (if (and allow-self (svn-status-line-info->directory-p line-info))
    ...
    (or (file-name-directory (svn-status-line-info->filename line-in        ".")))

(defun svn-status-set-user-mark (arg)
  "Set a user mark on the current file or directIIf th
...
marked, even if it is a directory."
  ...
  (let ((info (svn-status-get-line-information)))    (if i              (svn-status-apply-usermark t arg)
          (svn-status-next-line 1))
      (message "No file on this line - cannot se
  "RIf the cursor is on a file, this fiIf the cursor is on a directory, all files in this directory are unmarked.
If this function is called with a prefix argument, onunmarked, even if is a directory."
  (interactive "P")
  (setq arg (svn-status-possibly-negate-meaning-of-arg arg 'svn-status-set-user-mark))
  (let ((info (svn-status-get-line-information)))
    (if info
...
(defun svn-status-mark-changed (arg)
  "Mark all files that could be coThis * all modified files
* all files scheduled for addition
* all f
The last two categories include all copied and moIf called with a prefix ARG, unmark all such files."
...
(defvar svn-  "History list of regular expressions used in 
...
(defun svn-status-mark-filename-regexp (regexp &optional unmark)
  "Mark alIf   (interact   (list (svn-status-read-regexp (concat (i               (if current-prefix-arg t nil)))
  (svn-status-apply-usermark-check   '(lambda (info) (string-match regexp (svn
(defun svn-status-mark-by-file-ext (ext &optional unmark)
  "MarkIf the  (interactive
   (list (read-string (concat (if current-prefix-arg                                (sv   '(lambda (info) (let ((case-fold-search nil))
                    
(    (svn
(defun svn-status-toggle-hide  (in  (setq svn-status-hide-unmodified  (svn-status-update-buffer))

(defun svn-status-get-file-name-buffer-positio  "Find the buffer position for a file.
If the file is not found, retur                              (goto-char (previous-overlay-change cache            (found))
    ...
    (goto-c      (goto-char (next-overlay-change (point                                 (svn-status-get-line-information)))
        (setq start-pos (+ (point) svn-status-default-column))
        (setq f (defun  "Move the cursor the the line that displays NAME."
  (let ((pos                (svn-status-message 7 "
(defu  (let* ((st-info svn-status-info)
         (info))    (whi      (when (string= name (svn-status-line-info->filename (car s            i
...
or (if no files were marked) the file under point."
  (if (eq major-mode 'svn-status-mode)
    ...
    (if svn-status-get-line-information-for-file
        (list (svn-status-make-line-info (if (eq svn-status-get           
...
if no files have been marked."
  ...
  (loop for line-info in svn-status-info
        thereis (svn-status-line-info->has-usermark line-inf
(defun svn-s    ;;   (loop for line-info in svn-status-info
        thereis (and (not (svn-status-line-info->directory-p li                     (svn-status-line-info->has-usermark line-info))))

(defun svn-status-ui-information-hash-table ()
  (let ((st-info svn-status-info)
        (svn-status-ui-information (make-hash-t                         svn-status-ui-information))
      ...
      (if window-mode
          (progn
            ...
            (let ((buffer-read-only nil))
              ...
              (when scroll-to-to                  (when (eq window-mode t) ;; *svn-info* buffer
              (svn-info-mode))
            (other-window 1))
        ...
        (progn
          ...
          (with-current-buffer svn-status-last-output-buffer-name
            ...
            (setq version-string (buffer-substring-no-properties (point-min) (point-max))))
             (set-window-con 
(d  "Run `svn info' on all selected files.
See `svn-status-marked-files' for wha  (interactive)
  (svn-status-c  (sv
(defun svn-status-info-for-path (path)
  "Run svn info on the given PATH.
Return some interesting parts of the resulting output.
At the mom          (last-changed-author))
    (svn-run nil t 'info "info" path)    (with-current-buffer svn-process-buffer-name
                (setq last-change   
...
When called from a file buffer, go to the current line in the result  (interactive "P")
  (when current-pr    (setq revision (svn-  (unles  (setq svn-status-blame-revision revision)
  (setq svn-status-blame-file-name (if file-name
                                       file-                                     (svn-status-line-info->filename (svn-status-get-  (svn
(defun svn-blame-blame-again (arg)
  "Run svn blame again, using the revision before the change at point.
When point is at   (interactive "P")
  (let ((rev (svn-blame-rev-at-point)))
    (setq rev (number    (when current-prefix-arg
...
If there otherwise compare the working copy with BASE.
If ARG then prompt for revision to diff against (unless arg is '-)
When called with a ne  (interactive "P")
  (let ((non-recursive (or (and (numberp arg) (< arg 0)) (eq arg '-)))
        (revi    (svn-status-ensure-cursor-on-fil    (sv      
(defun svn-file-show-svn-diff (arg)
  "Run `svn diff' on the current file.
If there is a newer revision in the repository, the diff is done against HEAD,
otherwise compare the working copy with BAIf ARG then prompt for    (svn                                     (if arg :ask :auto)))

( If sothis is because marking a directory with \normally marks all oI  (i  (svn-status-show-svn-diff-            
(defun s  "Show the changeset foWhen ca  (let* ((upper-rev (if rev-against rev-against           (setq rev-arg (read-string "Revis    (svn-run nil t 'diff "diff" (concat "-r" rev-arg))
    (svn-status-activate-diff-mode)))
...
(defun svn-status-show-svn-diff-internal (line-infos recursive revision)
  ...
  (when (eq revision :ask)
    (setq revision (svn-status-read-revision-string
                    "Diff with files for version: " "P
  (setq svn-status-last-diff-optio
  (let ((clear-buf t)
        (beginning nil))
    (dolist (line-info line-infos)
      (svn-run nil clear-buf 'diff "diff" svn-status-default-diff-arguments
                   "-r" (if (eq revision :auto)
                            (if (svn-status-line-info->updat                                "                        (unless recursive "--non-recursive")
                   (svn-status-line-in
      ;; "svn diff --non-recursive" skips only s            ;; the user would have marked them if he wanted them to be compared.
      ;; So we'll look for t      ;; in the diff output, and delete it and everything that follows.
      ;; This is made more complicat                ;; Check `directory-p' relative to the `default-directory' of the
                           (goto-char (or beginnin                     (delete-region (   (sv
(defun svn-status-diff-save-current-defun-as-kill ()
  "Copy the function nameThat function uses `add-log-current-defun'"
  (interactive)
  (let ((func-name (add-l                 (message "Copied %S" func-name))
      ...
      (let ((original-name (svn-status-line-info->filename original))
        ...
        (cond
         ((or (eq original-filemarks ?M)  ;local mods: maybe do `svn mv --force'
          ...
          (if (yes-or-no-p
               (format "%s has local modifications; use `--force' to r                      (setq moved t))
                  ((eq original-filemarks ??) ;original is                                   original-name fallback original-name dest))
...
;;         ;; TODO: need to insert the new line-info in a sensible place, ie in the correct directory! [svn-status-filename-t
...
When called with a negative prefix argument, only update the selected files."
  ...
  (let* ((selective-update (or (and (numberp arg) (< arg 0)) (eq arg '-)))
         (rev (whe                                    (format "Selected entries: Run svn update -r ")
                           (format "Directory: %s: Run svn update -r " default-directory))
                         (if selective-update "HEAD" nil)))))
    (if selective-update
        (progn
          (message "Running svn-update for %s" (svn-status-marked              (message "Runn                (
...
(defun svn-status-pop-to-status-buffer ()
  ...
  (inte  (pop-to-buffer svn-status-buffer-name))

(defun svn-status-via-bookma  "AlRun `svn-status' on the sel  (interactive
   (list
    (let ((completion-ignore-case t))
      (funcall svn-status-completing-read-function "SVN  (unless bookmark
    (error "No bookmark specif  (le    (if (file-directory-p dire        (svn-status directory)
...
`svn-status-default-export-directory' is suggested as export directory."
  ...
  (let* ((src default-directory)
    ...
    (svn-run t t 'export "expo    (m
(defun svn-status-cleanup (arg)
  "Run `svn cleanup' on all selected files.SeWhen  (interactive "P")
  (let ((file-names (svn-status-get-file-    (if file-names
        (progn
...
See `svn-status-marked-files' for wh  (interactive)
  (let* ((marked-files (svn-status-marked-files))
         (num-of-files (length marked-files)))
    (when (yes-                   (format "Resolve %s? " (svn-status-line-             (form             (svn-run t t 'resolved "resolved" "--targe
(   (in  (svn-status-ensure-cursor-on-file)
  (let ((simple-path (svn-st      (      (setq simple-path (or (fi        (setq version (shell-command-to-string (concat "svnversion -n " full-path)))
    (message "svnversion for '%s': %s" simple-path version)
    version))

;; --------------------------------------------------------------------------------
;; Update the `svn-status-bu
...
(defun svn-status-after-save-hook ()
  "Set a                (svn-dir-len (length (or svn-dir "")))
        ...
        (while st-info
          ...
          (when (and (string= file-name i-fname)
            ...
            (svn-status-line-info->set-filemark (car st-info)
                                                svn-status-file-modified-after-save-flag)
            (save-window-ex                                 (pos (svn-status                  (if pos
                      (progn
                        (goto-char pos)
                        (delete-region (svn-point-at-bol) (svn-point-at-eol))
                        (svn-insert-line-in-status-buffer (car st-info))
                        (delete-char 1)                    (svn-status-message 3 "psvn: file %s not foun                         (svn-status-upd          (setq st-info   nil
(add-hook 'after-save-hook 'svn-status-after-save-hook)

;; --------------------------------------------------------;; vc;; --------------------------------------------------------------------------------
(defvar svn-status-sta(defvar svn-status-state-mark-tooltip nil) ; modeline tooltip display

(defun svn-status-state-mark-modeline-dot (color)
  (propertize "    "
              'help-echo 'svn-status-sta              'display
              `(image :type xpm
                      :data ,(format "/* static char * data[] = {
\"18 13 3 1\",
\"  c None\",
\"+ c #000000\",
\". c %s\",
...
(when svn-status-fancy-file-state-in-modeline
  ...
  (defadvice ediff-refresh-mode-lines
    ...
    ad-do-it
    (ediff-with-current-buffer ediff-buffer-A
                               (s    (ediff-with-current-buffer ediff-buffer-B
                               (svn-status-update-modeline))))

(defun svn-status-update-modeline ()
  "Update modeline state dot mark  (wh    (svn-status-update-state-mark
     (svn-status-interprete     
(defsubst svn-status-interprete-state-mode-color (stat)
  "Interpret vc-svn-state symbol to mode line color"
  (case     ('edited "tomato"      ('up-to-date "GreenYellow" )
              ...
              (progn
                ...
                (let ((content
                  ...
                  (let ((write-file-functions nil)
                         (save-buffer))))
            (find-file file-name-with-revision)))))
    ;;(message "default-directory: %s revision-file-info: %S" default-directory     (nreverse svn-status-get-specific-revi
(defun svn-status-ediff-with-revision (arg)
  "Run ediff on the current file with a diffIf therotherwise compare the working copy with BASE.
If ARG then prompt for revision to diff against."
  (interactive "P")
  (let* ((svn-status-get-specific-revision-file-info
          (svn-status-get-specific-revision-internal
           (list (svn-status-make-lin                  (file-relative-name
                                       (svn-status-line-i           (if arg :ask :auto)
           nil))
         (ediff-after-quit-destination-buffer (current-buffer))
         (default-directory (svn-status-base-dir))
         (my-buffer (find-file-noselect (c               (svn-transient-buffers (list my-buffer base-buff))
         (startup-hook '(svn-ediff-startup-    (ediff-buffers base-buff my-buffer startup-hook)))
...
(defun svn-status-propl        (defun svn-status-property-edit-one-ent  "Edit a property.
When called with a prefix argument, it is possible to enter a n  (interactive "P")
  (setq svn-status-proper  (svn-status-proplist-start))

(defun svn-status-property-se  (set  (svn-status-proplist-start))
          ...
          ((eq last-command 'svn-status-property-delete)
           ...
           (unless (string= prop-name "")
             (save-excursion
               ...
               (let ((file-names (svn-status-marked-file-names)))
                 (when file-names
                   (message "Going to delete prop %s fo   
(defun  (let* ((commit-buffer (get-buffer-create "*                   (message "Edit property %s fo    (    (save-excursion
      (set-buffer svn-process-buffer-name)
    ...
    ;; only the accessible part.  So     (d    (setq default-directory dir)
    (insert prop-value)    (      (when (listp new-prop-value)
        (if remove-values
...
When called with the prefix arg -, remove Id from the svn:keywords property."
  (interactive "P")
  (svn-status-property-edit (svn-status-marked-files) "svn:keywords" '("Id") (eq 
...
(defun svn-status-prope  "Set W  (in  (if unset
      (progn
        (svn-run nil t 'propdel (append (list "propdel" "svn:executable")              (svn-stat    (svn-stat
(defun svn-sta  "Set the svn:mime-type property on the marked files."
  (  (re  (let ((completion-ignore-cas           (svn-status-property-set-property
     (svn-status-marked-files) "svn:mime-type"
     (funcall svn-status-completing-read-function  
;; sv;; ---------------------------------------------------------
((
(when  (setq svn-prop-e   (def  (define-key svn-prop-edit-mode-map [(control ?c) (control ?l)] 'svn-  (define
(easy-menu-define svn-prop-edit-mode-menu svn-pro"'svn-prop-edi                  '("SVN-PropEdit"
                    ["Commit" svn-prop-edit                                     
(defun svn-prop-edi  "Major Mode to edit file properties of filComma\\{svn-prop-edit-mode-map}"
   (ki  (use-local-map svn-prop-edit-mode-map)
...
(defun svn-prop-edit-do-it (async)
  "Run svn propset `svn-status-propedit-property-name' with the co*svn-property-edit* buffer."
  (message "s                (mapcar '     (se    (when (fboundp 'set      (set-buffer-file-codin    (setq svn-st          (concat svn-status-temp-dir "svn-prop-edit.txt" svn-temp-suffix))
     (wh    (svn-statu                     (setq svn-                    "--targets" svn-status-temp-ar                    '("--encoding"          (unless async (svn-status-remove-temp-file-mayb  (when svn-status-pre-prop    (set-window-configuration svn-status-pre-propedi
(defun svn-prop-edit-svn-diff (arg)
  (interactive "P")
  (set-buffer svn-status-buffer-name)
  ;; Because propedit is not recursive in our use, neither is this  (svn-status-show-svn-diff-i (defu  (interactive "P")
  (set-buffer svn-status-buffer-name)
...
(if svn-log-edit-use-log-edit-mode
    (define-derived-mode svn-log-edit-mode log-edit-mode "svn-log-edit"
      ...
      (easy-menu             (set (make-local-variable 'log-edit-listfun) 'svn-log-edit-files-to-com      (      (                    (substitute-command-key   (def    "Major Mode to edit svn log messages.
Commands:
...
(when (not svn-log-edit-mode-map)
  ...
  (define-key svn-log-edit-mode-map (kbd "C-c C  (define-key svn-log-edit-mode-map (kbd "C-c C  (define-key svn-log-edit-mode-map (kbd "C-c C-q") 'svn-log-edit-a
...
"'svn-log-edit-mode' menu"
                  '("SVN-Log"
                    ...
                    ["C                    ["Show Diff" svn-log-edit-svn-diff t]
...
"
  ...
  (let* ((short-file-names (and (numberp prefix-arg) (< prefix-arg 0)))
         ...
         (this-defun (or (add-log-current-defun)
                         (save-excursion
                                                              (c-be                                   (search-forward "(" nil t)
                                   (forward-char -1)
                                                          (point)               (log-f    (find-file log-file)
    (goto-char (point-min))
    ;; Strip text properti      ;    (if (and
         this-defun                            (save-excursion                   ;; and it's about the same file
           (save-match-data
             (if (re-search-backward  ; Ick, I want a real filename regexp!
                                 (string-equal (match-string 1                (if (looking-at " ") (forward-char 1)))
      ;; Else no log message for this defun in progress.             (if (search-forward this-file n                    (if (looking-        ;; Found neither                 ;; type it themselves.
        (if (not this-def
;; ------------------------------------------;; sv;; --------------------
(def(put 'svn-log-view-mode-map 'risky-local-variable
(when (not svn-log-view-mode-map)
  ...
  (define-key svn-log-view-mode-map (kbd "~") 'svn-log-get-  (define-key svn-log-view-mode-map (kbd "f") 'svn-  (def  (define-key svn-log-view-mode-map (kbd "=") '  (define-key svn-log-view-mode-map (kbd "#") 'svn-  (defi  (define-key svn-log-view-mode-map (kbd "TAB") 'svn-log-next-link)
  (de    (de
...
(defun svn-log-vie  (int  (mouse-set-point event)
  (let* ((rev (svn-log-revision-at-point)))
    (when rev
      (svn-status-face-set-temporary-during-popup
       'svn-status-marked-popup-face ( 
(defvar svn-log-view-font-lock-basic-keywords
  '(("^r[0-                                  (
((defi  "Major Mode to show the output from svn log.
Commands:
...
(when (not svn-log-link-keymap)
  ...
  (define-key svn-log-link-ke
(defun svn-log-resolve-mouse-lin  (interactive "e")
  (mouse-set-point event)
...
(defun svn-log-resolve-link ()
  ...
  (let* ((point-adjustment (if (not (get-text-property (- (point) 1) 'link-handler)) 1
         ...
         (link-name (buffer-substring-no-properties (previous-single-property-change (+ (point) point-adjustment) 'li     (
(defun svn-log-register-link-handler (handler-id   "Register a link handler for external links His reLINK-REGEXP specifies a regular expression that matches the external link.
HANDLER-FUNCTION is called with the match o  (le                                                                                        (
...
(defun svn-log-resolve-trac-ticket-short (link-name)
  ...
  (interactive)
  (let ((ticket-nr (string-to-number (svn-substring-no-pro    (
...
;; the
;; ------------------------------------------------;; ----(defvar svn-info-mode-map () "Keymap used in(put 
(when (not svn-info-mode-map)
  (setq svn-info-mode-map (make-sparse-keymap)  (de  (define-key svn-info-mode-map (kbd "h") 'svn-status-pop-to-partner-buffer)
  (define-key svn-info-mode-map (kbd "n"   (def  (define-key svn-info-mode-map [?q] 'bury-buffer
(defu  "Major Mode to view informative output from svn."    (se  (setq mode-name "svn-info")
  (toggle-read-only 1))
...
Currently is the output from the svn update command known."
  ...
  (cond ((save-excursion
         ...
         (let ((cur-pos (point))
               ...
               (pos))
           (when (eq system-type 'windows-nt)
             (setq file-name (replace-regexp-in-string "\\\\" "/" fil                (with-current-buffer svn-status-buffer-name
             (s           (when pos
             (svn-status-pop- 
;; -----------------------------------------------------------;; svn;; ---------------------------
(  (se        (cons (list 'svn-blame-mod              minor-mode-alist)))

(defvar svn-blame-mode-map () "Keymap used in `s(put 
(when (not svn-blame-mo  (setq svn-blame-mode-map (make-sparse-keymap))
  (define-key svn-blame-mode-map [?s  (define-key svn-blame-mode-map (kbd "n") 'next-line)
  (define-key svn-blame-mode-map (kbd "p") 'p  (  (de  (define-key svn-blame-mode-map (kbd "r") 'svn-blame-highli  (define-k   (def  (def  (define-key svn-b
(easy"svn blame minor mode menu"
                  '("SvnBlame"
                    ["Jump to source location" svn-blame-open-source-file t]
                    ["Show changeset" svn-blame-show-changeset t]
                                       ["Sho                    ["S                         ["Highlight by revisi
(or     (setq minor-mode-map-alist
...
It is probably because the revisions are in 'before-st
Key bindings:
\\{svn-blame-mode-map}"
  (interactive "P")
  ...
  (if svn-blame-mode
      (progn
        (easy-menu-add svn-     (easy-menu-remove svn-blame-mode-menu))
  (force-mode-line-update))

(defun svn-status-act T    (    (let ((buffer-read-only nil)
...
(defun svn-blame-show-statistics ()
  ...
  (let ((author-map (make-hash-table :test 'equal))
    ...
    (save-excursion
      (goto-char (point-min))
      (while (not (eobp))
        (dolist (ov (overlays-in                                 (+ (gethash au               (pop-to-buffer (get-buffer-create (replace      (erase-buffer)
      (dolist (line (sort author-list '(lambda (v1 v2) (> (cadr v1) (        
(defun   (cadr (overlay-get ov 'rev-info)))

(defun svn-blame-highlight-author ()
  "(Un)Highlight all lines with the same author."
  (interactive)
  (svn-blame-highlight-line-maybe 'svn-blame-highlight-author-fi
(defun svn-blame-highlig  (ca
(defun svn-blame-highlight-  "(Un)Highlight all line  
;; --;; svn-process-mode
;; --------------------------------------------------------------------------------
(defvar svn-process-mode-map () "Keymap used in `svn-process-mode' buff(put 
(when (not svn-process-mode-map)
  (setq   (define-key svn-process-mode-map (kbd  (def  (define-key svn-proces
(easy-menu-define svn-process-mode-menu svn-process-mode-map
"'svn-process-mode' menu"
                  '("SvnProce                                                      ["Bury process buffer" bu
(defun svn-process-mode ()  "Major Mode to view process 
You can send a new liYou can send raw data to the process v  (interactive)
  (kill-all-local-variables)
  (use-local-map svn-process-mode-map)
  (easy-menu-add svn-log-view-mode-menu)
  (setq major-mode 'svn-process-mode)
  (setq mode-name "svn-pr
;; ----------------------------------------------------------;; svn status persistent option;; -------------------------(  "Fi  (let ((old-process-default-dir))    (with-          (setq default-directory directory)) ;; update  (svn-run nil     ;;    (setq default-directory old-process-default-d    (goto-char (point-min))
    (let ((case-fo                  
...
Return nil, if not in a svn working copy."
  (let* ((start-dir (expand-file-name (or start-directory default-directory         ;;(message "svn-status-base-dir    (if (not (eq base-dir 'not-found))
        base-dir
      ...
      (let* ((base-dir start-dir)
        ...
        (if (and (<= (car svn                (while (when (and dir-below (file-exists-p dot-svn-dir))
                                (setq                         (and                                     (when dir-below
                     (if (st               (setq base-dir (and in-tree base-dir              (svn-status-message 7 "svn-status-base-dir  
(defun svn-status-base-dir-for-ancient-svn-client (&optio  "Find the svn root directRThis f  (let* ((base-dir (expand-file-name (or sta                (dir-below    (             (setq base-dir (file-name-             (string-match "\\(.+/\\).+/" dir-below)
             (                  (setq dot-svn-dir (concat dir-below (svn-w    (and in-tree base-dir)))

(defun svn-status-save-state ()
  "Save psvn persistent options for this worki      (er    ;; TO CHECK: why is svn-status-options a global variable??
    (setq svn-status-options
                          (list "sort-status-buffe           (list "elide-list"           (li           (list "branch-list"                 ))
...
(defun svn-status-load-state (&optional no-error)
  ...
  (interactive)
  (let (    (        (with-temp-                  (setq svn-trac-project               (setq svn-status-elided-list
                (nth 1 (assoc "elide-list" svn-status-options)))
          (se                (nth 1           (se               (setq svn-status-changelog-style
                  (when (an               (setq svn-trac-project-root nil
                svn-status-elided-list nil
                svn-status-module-name nil
                svn-status-branch-list nil
                svn-status-changelog-style 'changelog)
        (error "psvn.el: %s is not readable." fil
(defun svn-status-toggle-sort-status-buffe  "Toggle
If you turn off sorting, you can speed up \\[svn-status]the buffer is not correctly sorted then.  This function will be
...
(defun svn-admin-create (dir)
  ...
  (interactive (list (expand-file-name
                      (svn-read-directory-name "Create a svn repository at: "
                                               svn-admin-default-create-directory nil  (sh  (setq svn-admin-last-r  
;; - Import an empty directory
;;   cd to an empty direc;;   svn import -m "Initial import" . file:(defu  "Import an empty trunk directory to `svn-admin-last-repository-dir'.
Set `svn-admin-last-  (in  (let ((empty-temp-dir-name (make-temp-name svn-status-temp-dir)))
...
After that step the empty base directory (either the root dthe trunk directory of the selected reworking directory."
  (interactive)
  (if (y-or-n-p "Create local repository? ")
    ...
    (setq svn-admin-last-repository-dir (read-string "Reposito  (svn-checkout svn-admin-last-repository-dir "."))

;; -------------------------;; sv;; ---------------------------------------------------------------;;; N;;  (load;;;;  ;;  M-x elp-results
...
(defun svn-status-elp-init ()
  ...
  (message "Run the desired svn command (e.g. M-x s
(defun svn-status-last-commands (&optiona   (un    (setq string-prefix ""))
  (with-output-to-string
    (dolist (e (ring-elements svn-last-cmd-ring))
      (princ (format "%s%s: svn %s <%s>\n" s
;; --------------------------------------------------------------------------------
;;; ---(defun svn-insert-indented-lines (text)
  "Helper function to insert TEXT, indented by two  (dol    (insert (format "  %s\n" line))))

(defun svn-prepa  "Cr  (interactive)
  (let* ((last-output-buffer-name (or svn-status-last-output-buffer-name svn-process-buffer-name))
    ...
    (insert "Please enter a     (insert "and send    (insert "Rev    (    (insert "Language environment:\n    (doli      (w        (insert (format "  %s\n" elem))))
    (s    (insert (format "\nContent of the <%s> buffer:\n" last-output-buff    (svn-    (
;; ------------------------------------------------------;; Make it easier to reload psvn, if a dis;; ;; (l
;; Note the above will only work, if the loaded psvn.el has already the
;; function svn-prepar;; I;; (load "/path/to/psvn.el");;make svn-prepare-for-reload available
;; (svn-prepare-for-reload)
;; (load "/path/to/psvn.el");; update the keybindings
;; --------------------------------------------------------------
((                "A (defun svn-prepare-for-reload ()
...
A variable will keep its value, if it is s  (interactive)
  (dolis    (      (message (format "Resetting value of %s to nil" var)))
      (set var nil
(provide 'psvn)
